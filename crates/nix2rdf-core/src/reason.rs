...
use oxrdf::{Literal, NamedNode, Quad, Subject, Term};
...
fn read_input(store: &Store, path: &Path) -> Result<InputGraph> {
    let abs = if path.is_absolute() { path.to_path_buf() } else { store.root().join(path) };
    ...
    Ok(InputGraph { graph: NamedNode::new(graph)?, path: abs, nquads })
}
...
pub fn fragment_for_iri(iri_str: &str) -> Option<FragmentKind> {
    ...
    Some(match kind {
        ...
        "git" => {
            ...
            FragmentKind::Commit { repo: percent_decode(repo), sha: sha.to_string() }
        }
        "k8s" => {
            let (cluster, rest) = id.split_once('/')?;
            let (k, h) = rest.split_once('/')?;
            match k {
                "snapshot" => FragmentKind::K8sSnapshot { cluster: percent_decode(cluster), hash: h.to_string() },
                "nodes" => FragmentKind::K8sNodes { cluster: percent_decode(cluster), hash: h.to_string() },
                _ => return None,
            }
        }
        _ => return None,
...
fn decode_nar(seg: &str) -> String {
    ...
    if let Some((algo, b)) = seg.split_once('-') {
        if let Ok(bytes) = base64::engine::general_purpose::URL_SAFE_NO_PAD.decode(b) {
            return format!("{algo}-{}", base64::engine::general_purpose::STANDARD.encode(bytes));
        }
    }
    seg.to_string()
}
...
pub fn select_inputs(store: &Store, sel: &InputSelection) -> Result<Vec<InputGraph>> {
    ...
    while let Some(p) = queue.pop() {
        ...
        if follow {
            ...
            for pred in [t::closureContains(), t::hasRoot(), t::snapshot(), t::inputSrc(), t::imageDerivation()] {
            ...
            for pred in [crate::vocab::k8s_terms::runsImage(), crate::vocab::k8s_terms::hostGeneration()] {
                for o in objects_of(&ig.nquads, &pred) {
                    if let Some(k) = fragment_for_iri(&o) {
                        refs.push(k);
                    }
...
pub fn input_hash(inputs: &[InputGraph]) -> String {
    let mut items: Vec<(String, String)> = inputs.iter().map(|i| (i.graph.as_str().to_string(), hash::sha256_hex(i.nquads.as_bytes()))).collect();
    items.sort();
...
pub fn build_program(packs: &[Pack], input_path: &Path) -> Result<String> {
    ...
    for (p, u) in iri::PREFIXES {
        all_prefixes.entry(p.to_string()).or_insert_with(|| u.to_string());
    }
    ...
    prog.push_str(&format!("@import raw :- rdf{{resource = \"{}\"}} .\n", input_path.display()));
...
pub fn canonicalize_nulls(triples: &[DerivedTriple], derived_graph: &NamedNode) -> Vec<(Term, NamedNode, Term)> {
    ...
    for tr in triples {
        ...
        for term in [&tr.s, &tr.o] {
            if let RdfTerm::Null(id) = term {
                ...
                sigs.entry(id.clone()).or_default().insert(format!("{pos} {line}"));
            }
        }
    }
    let null_iri: BTreeMap<String, NamedNode> = sigs
        ...
        .map(|(id, sig)| {
            ...
            (id, NamedNode::new_unchecked(format!("{}/n/{}", derived_graph.as_str(), hash::short(&h))))
        })
        .collect();
    let conv = |t: &RdfTerm| -> Term {
        match t {
            RdfTerm::Iri(i) => Term::NamedNode(NamedNode::new_unchecked(i.clone())),
            RdfTerm::Literal(l, d) => Term::Literal(Literal::new_typed_literal(l.clone(), NamedNode::new_unchecked(d.clone()))),
            RdfTerm::LangString(l, g) => Term::Literal(Literal::new_language_tagged_literal_unchecked(l.clone(), g.clone())),
    ...
    for tr in triples {
        ...
        out.push((conv(&tr.s), NamedNode::new_unchecked(p.clone()), conv(&tr.o)));
    }
    out.sort_by(|a, b| a.0.to_string().cmp(&b.0.to_string()).then(a.1.cmp(&b.1)).then(a.2.to_string().cmp(&b.2.to_string())));
    out.dedup();
    out
}
...
pub fn run(store: &Store, pack_list: &[Pack], inputs: &[InputGraph], force: bool) -> Result<RunResult> {
    let ruleset = packs::ruleset_hash(pack_list);
    let ih = input_hash(inputs);
    let kind = FragmentKind::Derived { ruleset: ruleset.clone(), input: ih.clone() };
    ...
    if store.exists(&kind) && !force {
        ...
        return Ok(RunResult { ruleset_hash: ruleset, input_hash: ih, derived, run, inputs: inputs.len(), derived_facts: 0, already_existed: true });
    }

    let tmp = tempfile::Builder::new().prefix("nix2rdf-reason-").tempdir().map_err(|e| Error::Reason(e.to_string()))?;
    let input_path = tmp.path().join("input.nq");
    {
        let mut f = std::fs::File::create(&input_path).map_err(|e| Error::io(&input_path, e))?;
        for i in inputs {
            f.write_all(i.nquads.as_bytes()).map_err(|e| Error::io(&input_path, e))?;
        }
        f.write_all(vocab_quads(pack_list)?.as_bytes()).map_err(|e| Error::io(&input_path, e))?;
    }
    ...
    let run = run_fragment(&ruleset, &ih, pack_list, inputs, derived.len());
    Ok(RunResult { ruleset_hash: ruleset, input_hash: ih, derived, run, inputs: inputs.len(), derived_facts: out.derived_facts, already_existed: false })
}

fn run_fragment(ruleset: &str, input: &str, pack_list: &[Pack], inputs: &[InputGraph], quads: usize) -> Fragment {
    let derived_graph = FragmentKind::Derived { ruleset: ruleset.into(), input: input.into() }.graph();
    let mut f = Fragment::new(FragmentKind::Run { ruleset: ruleset.into(), input: input.into() });
    ...
    f.add_typed(derived_graph.clone(), t::ranAt(), &chrono::Utc::now().to_rfc3339_opts(chrono::SecondsFormat::Secs, true), xsd_date_time());
    f.add_int(derived_graph.clone(), t::derivedQuadCount(), quads as i64);
    f.add_str(derived_graph.clone(), t::nemoVersion(), nemo_engine::NEMO_VERSION);
...
pub fn gc_derived(store: &Store, graph: Option<&Graph>, keep_rulesets: &[String]) -> Result<usize> {
    ...
    for sub in ["derived", "runs"] {
        for p in store.fragment_paths_under(sub)? {
            ...
            let ruleset = rel.components().nth(1).map(|c| c.as_os_str().to_string_lossy().to_string()).unwrap_or_default();
            if keep_rulesets.iter().any(|k| k == &ruleset) {
                continue;
            }
            if sub == "derived" {
                if let Some(g) = graph {
                    let input = p.file_name().unwrap().to_string_lossy().trim_end_matches(".nq.zst").to_string();
                    let _ = g.drop_graph(&FragmentKind::Derived { ruleset: ruleset.clone(), input }.graph());
                }
            }
