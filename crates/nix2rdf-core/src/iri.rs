...
pub fn sparql_prologue() -> String {
    PREFIXES.iter().map(|(p, u)| format!("PREFIX {p}: <{u}>\n")).collect()
}
...
pub fn encode_segment(s: &str) -> String {
    ...
    for b in s.bytes() {
        match b {
            b'A'..=b'Z' | b'a'..=b'z' | b'0'..=b'9' | b'-' | b'.' | b'_' | b'~' | b'@' | b'+' | b'!' | b',' | b'=' | b':' => {
                out.push(b as char)
            }
...
pub fn drv(store_hash: &str) -> NamedNode { id(&format!("drv/{store_hash}")) }
pub fn out(store_hash: &str) -> NamedNode { id(&format!("out/{store_hash}")) }
pub fn src(nar_hash_sri: &str) -> NamedNode { id(&format!("src/{}", nar_hash_segment(nar_hash_sri))) }
pub fn pin(nar_hash_sri: &str) -> NamedNode { id(&format!("pin/{}", nar_hash_segment(nar_hash_sri))) }
pub fn flake_input(root_nar_hash_sri: &str, parent_node_id: &str, input_name: &str) -> NamedNode {
    id(&format!(
        "pin/{}/input/{}/{}",
        nar_hash_segment(root_nar_hash_sri), encode_segment(parent_node_id), encode_segment(input_name)
    ))
}
pub fn pin_attr(root_nar_hash_sri: &str, attr_path: &str) -> NamedNode {
    id(&format!("pin/{}/attr/{}", nar_hash_segment(root_nar_hash_sri), encode_segment(attr_path)))
}
pub fn nixpkgs(rev: &str) -> NamedNode { id(&format!("nixpkgs/{rev}")) }
...
pub fn package_version(attr_path: &str, version: &str) -> NamedNode {
    id(&format!("pkgver/{}/{}", encode_segment(attr_path), encode_segment(version)))
}
pub fn release(name: &str) -> NamedNode { id(&format!("release/{}", encode_segment(name))) }
pub fn commit(repo: &str, sha: &str) -> NamedNode { id(&format!("git/{}/{sha}", encode_segment(repo))) }
pub fn gen(system_hash: &str) -> NamedNode { id(&format!("gen/{system_hash}")) }
pub fn nixos_configuration(flake_nar_hash_sri: &str, name: &str) -> NamedNode {
    id(&format!("pin/{}/nixosConfigurations/{}", nar_hash_segment(flake_nar_hash_sri), encode_segment(name)))
}
pub fn option(path: &str) -> NamedNode { id(&format!("option/{}", encode_segment(path))) }
...
pub fn option_definition(gen_hash: &str, path: &str, index: usize) -> NamedNode {
    id(&format!("gen/{gen_hash}/opt/{}/def/{index}", encode_segment(path)))
}
pub fn value(hash: &str) -> NamedNode { id(&format!("value/{hash}")) }
pub fn image(sha256_hex: &str) -> NamedNode { id(&format!("oci/sha256/{sha256_hex}")) }
pub fn layer(sha256_hex: &str) -> NamedNode { id(&format!("oci/layer/sha256/{sha256_hex}")) }
pub fn pack(content_hash: &str) -> NamedNode { id(&format!("pack/{content_hash}")) }
pub fn derived(ruleset_hash: &str, input_hash: &str) -> NamedNode {
    id(&format!("derived/{ruleset_hash}:{input_hash}"))
}
pub fn fragment(kind: &str, ident: &str) -> NamedNode { id(&format!("fragment/{kind}/{ident}")) }
pub fn policy(pack: &str, name: &str) -> NamedNode { nn(format!("{POLICY_BASE}{pack}/{name}")) }

// ----- k8s: instances -------------------------------------------------------

pub fn k8s_cluster(cluster_id: &str) -> NamedNode { id(&format!("k8s/cluster/{}", encode_segment(cluster_id))) }
pub fn k8s_obj(cluster_id: &str, kind: &str, ns: Option<&str>, name: &str) -> NamedNode {
    match ns {
        Some(ns) => id(&format!("k8s/{}/{kind}/{}/{}", encode_segment(cluster_id), encode_segment(ns), encode_segment(name))),
        None => id(&format!("k8s/{}/{kind}/{}", encode_segment(cluster_id), encode_segment(name))),
    }
}
pub fn k8s_node(cluster_id: &str, name: &str) -> NamedNode { k8s_obj(cluster_id, "node", None, name) }
pub fn k8s_namespace(cluster_id: &str, name: &str) -> NamedNode { k8s_obj(cluster_id, "ns", None, name) }
pub fn k8s_container(cluster_id: &str, kind: &str, ns: &str, name: &str, container: &str) -> NamedNode {
    id(&format!(
        "k8s/{}/{kind}/{}/{}/c/{}",
        encode_segment(cluster_id), encode_segment(ns), encode_segment(name), encode_segment(container)
    ))
}
pub fn k8s_snapshot(cluster_id: &str, content_hash: &str) -> NamedNode {
    id(&format!("k8s/{}/snapshot/{content_hash}", encode_segment(cluster_id)))
}
pub fn k8s_nodes_fragment(cluster_id: &str, content_hash: &str) -> NamedNode {
    id(&format!("k8s/{}/nodes/{content_hash}", encode_segment(cluster_id)))
}
pub fn k8s_failure_domain(cluster_id: &str, kind: &str, value: &str) -> NamedNode {
    id(&format!("k8s/{}/domain/{kind}/{}", encode_segment(cluster_id), encode_segment(value)))
}
pub fn k8s_permission(cluster_id: &str, api_group: &str, resource: &str, verb: &str) -> NamedNode {
    id(&format!(
        "k8s/{}/permission/{}/{}/{}",
        encode_segment(cluster_id),
        encode_segment(if api_group.is_empty() { "core" } else { api_group }),
        encode_segment(resource),
        encode_segment(verb)
    ))
}
pub fn owner(label_value: &str) -> NamedNode { id(&format!("org/owner/{}", encode_segment(label_value))) }
...
mod tests {
    ...
    #[test]
    fn segment_encoding() {
        assert_eq!(encode_segment("python3Packages.requests"), "python3Packages.requests");
